...
contract('PokerTournament', async accounts => {
  ...
  describe('interaction', () => {
    ...
    it('should return correct prize pool', async () => {
      ...
      assert.equal(
        await pokerTournament.prizePool(),
    ...
    it('should return correct amount of players that has voted', async () => {
      ...
      await pokerTournament.voteForWinner(
  ...
  describe('helpers', () => {
    it('should not be equal', async () => {
      let isNotEqual = await pokerTournament.isEqual(
        [`${accounts[0]}`],
        [`${accounts[1]}`]
      );
    ...
    it('should be equal', async () => {
      let isEqual = await pokerTournament.isEqual(
        [`${accounts[0]}`],
        [`${accounts[0]}`]
      );
    ...
    it('getPrizeCalculation', async () => {
      assert.equal(
        (await pokerTournament.getPrizeCalculation(1, 2, 100, {
          from: accounts[0]
        })).toNumber(),
        66
      );
      assert.equal(
        (await pokerTournament.getPrizeCalculation(2, 2, 100, {
          from: accounts[0]
        })).toNumber(),
        33
      );
      assert.equal(
        (await pokerTournament.getPrizeCalculation(1, 5, 100, {
          from: accounts[0]
        })).toNumber(),
        51
      );
      assert.equal(
        (await pokerTournament.getPrizeCalculation(2, 5, 100, {
          from: accounts[0]
        })).toNumber(),
        25
      );
      assert.equal(
        (await pokerTournament.getPrizeCalculation(3, 5, 100, {
          from: accounts[0]
        })).toNumber(),
        12
      );
      assert.equal(
        (await pokerTournament.getPrizeCalculation(4, 5, 100, {
          from: accounts[0]
        })).toNumber(),
        6
      );
      assert.equal(
        (await pokerTournament.getPrizeCalculation(5, 5, 100, {
          from: accounts[0]
        })).toNumber(),
        3
      );
    ...
    it('getPotiumSize', async () => {
      ...
      assert.equal((await pokerTournament.getPotiumSize()).toNumber(), 1);
      for (let i = 5; i < 10; i++) {
        await pokerTournament.deposit({ from: accounts[i], value: 0 });
      }
      assert.equal((await pokerTournament.getPotiumSize()).toNumber(), 2);
      for (let i = 10; i < 15; i++) {
        await pokerTournament.deposit({ from: accounts[i], value: 0 });
      }
      assert.equal((await pokerTournament.getPotiumSize()).toNumber(), 3);
    });
  });
  ...
  describe('limitations', async () => {
    ...
    it('should throw an error if an outside accounts tries to vote', async () => {
      await truffleAssert.reverts(
        pokerTournament.voteForWinner([`${accounts[0]}`], {
          from: accounts[0]
        }),
    ...
    it('should throw an error if the same address tries to vote again', async () => {
      ...
      await pokerTournament.voteForWinner([`${accounts[0]}`], {
        from: accounts[0],
        gas: 3000000
      });
    ...
    it('should throw an error if the amount of address voted for doesnt match potium size', async () => {
      await pokerTournament.deposit({ from: accounts[0], value: VALUE });
      await truffleAssert.reverts(
        pokerTournament.voteForWinner([`${accounts[0]}`, `${accounts[1]}`], {
          from: accounts[0]
        }),
